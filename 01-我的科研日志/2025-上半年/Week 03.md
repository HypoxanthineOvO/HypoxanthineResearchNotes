...
2. 讲解 Instant-NGP
3. GS-Core
王珺说他想找时间参与一下，我觉得可以拉他一起来看看混合渲染器？
下周讲 Deferred Shading 好了

# 3.7（周五）
## 1. 和浪潮的 Meeting
浪潮那边在算法上的能力不太够，因此这个项目还是我们这边来进行 Demo。
那么从下周开始就直接开始做渲染器的第一步好了

# 3.8（周六）& 3.9（周日）
## 1. 帮康杰跑数据
康杰的论文需要一些 Ray Casting 的基础数据
## 2. 浮点数精度转换器
简单实现了一下，似乎他的版本并没有实现好 Subnormal To Subnormal 的转换。
其他的部分都比较符合逻辑，但内在细节比较复杂。
下周抽空把功能融入进 ASIC_Sim 框架里。
- 后续要注意把定点数的相互赋值部分增加截断相关的处理。

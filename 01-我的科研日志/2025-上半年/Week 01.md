...
### 3.1. Ray Casting 的统计
- 把代码单独拉出来了一份，方便修改。
- 要考虑估算一个采样点多少密度比较合适，总之现在估算一根非空光线平均用四个点（其实道理上已经很多了）
- 整个 NGP 都有两到三倍的冗余。比较好写应该是。


# 2.22（周六）
# 1. 368 月赛
虽然五场都输了，但是收获颇丰
- 第一点是，即使面对分数很高的对手，也开始敢斗敢想办法了，即使打 1700+ 乃至于 1800+ 的对手，可能也能通过各种战术和策略拿下一些分数
- 第二点是明确了自己的弱点：最大的两个弱点就是接发轮的长球处理（现在斗短感觉应该有 1600 多一点的水平，但长球约等于不会接）和相持中的步伐问题导致的抢点。
	- 现在反而是直接起板失败的没那么多了，无非就是起板质量不够高，和后续衔接不上
- 第三点是在最后一局里找到了一点收紧核心的感觉，加以训练必然能让正手有突破性的进展
	- 干他妈的，苦练！
# 2. 论文阅读
应该看 EDSR & MDSR。
...
# 2.23（周日）
# 1. 论文写作
准备写一下 Ray Casting 部分的 Profiling 部分。
- 简单进行了写作，感觉难度并不大，明天要开始设法写投影方案的优化部分和画图部分了。
# 2. 乒乓球训练
- 今天体力不好，但有录像之后感觉会对自己的训练效果形成正面增益。
- 似乎找到了正手的发力核心，除了身上之外，和手指手腕？
- 今天训练的视频还是能找出做的好的，希望能慢慢增加这些点的比例
# 3. 论文阅读
今天读的是 MSRN，但感觉这种傻逼深度学习论文读的必要性实在不高。
缺乏必要的数学推导和理论验证，纯纯一堆网络模块的拼凑，理论解释全是推测和感觉，这 nm 读的有啥用？
明天看看用 GAN 和 Transformer 的差不多得了，真难蚌啊